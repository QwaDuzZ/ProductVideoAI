// Edge Function: start-outro
// ТЗ §5.7 — Smart Outro: fade/loop
...
serve(async (req) => {
  ...
  try {
    ...
    const { project_id, video_url, master_audio_duration_ms, outro_mode } = body;

    if (!project_id || !video_url || !master_audio_duration_ms) {
    ...
    const { data: mediaTask, error: taskError } = await supabase
      ...
      .insert({
        ...
        payload: {
          ...
          mode: outro_mode ?? "fade",
          output_path: `results/${project_id}/with_outro.mp4`,
        },
        status: "pending",
      })
      .select("id")
